---
title: TensorRT-LLM 部署 ChatGLM3
authors: initencunter
tags: [docs, 教程]
---

## 安装 [TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM)

[windows安装教程](https://nvidia.github.io/TensorRT-LLM/installation/windows.html)

## 下载模型

```shell
git clone git clone https://www.modelscope.cn/ZhipuAI/chatglm3-6b.git chatglm3_6b
```

## 转换模型

```shell
## 如果提示找不到动态链接库，可以到仓库以外的目录来转换
...
## 单次推理
```shell
python ../run.py --input_text "你好" --max_output_len 1000  --tokenizer_dir chatglm3_6b --engine_dir trt_engines/chatglm3_6b/fp16/1-gpu
```

## 连续推理

将 `../run.py` 改成下面的代码，然后运行
```shell
python ../run.py --max_output_len 1000  --tokenizer_dir chatglm3_6b --engine_dir trt_engines/chatglm3_6b/fp16/1-gpu
```
